...
function actOpen(resId, actIdx) {
  ...
  //data.context.group.id.match(/^(IS0017Fall2019|WentworthFall2019)/)
  if(is_quizjet_url){
 ...
 // }
  ...
  if(is_quizjet_url)
    ui.vis.act.frame.src = act.url + "&grp=" + state.curr.grp + "&usr=" + state.curr.usr + "&sid=" + state.curr.sid + "&cid=" + state.curr.cid + traceParams;
  else
...
function updateLearnerDataWithOtherEstimates(item_kc_estimates){
    console.log("updateLearnerDataWithOtherEstimates");
...
function visGenGrid(cont, gridData, settings, title, tbar, doShowYAxis, doShowXLabels, sqHFixed, cornerRadius, topicMaxW, xLblAngle, extraPaddingB, isInteractive, miniVis, miniSettings, resNames, doShowResNames, doShowTimeline, doReserveTimelineSpace, doUpdActLstTopicCellX, helpId) {
  ...
  if(state.args.uiIncenCheck){
      ...
      if (state.curr.grp.startsWith("IS0017Spring2020")){ //TODO: make this parameterized !!!
        full_points = 1;
      }

      if(state.curr.grp.startsWith("IS0017Spring2020")){//TODO: Change this for a parameter with the id of the resource
      ...
      g.append("svg:image")
        ...
        .filter(function(d) {return d3.select(this).node().parentNode.parentNode.getAttribute("data-grid-name")=="me" && d.actIdx==-1 && d.resIdx==0 && d.topicIdx>0 }) //first if is for just showing the checkmarks on the "Me" row in MG, not in all of the other two (me vs group and group)
          ...
          .attr("xlink:href", function(d){
                    ...
                    if(topic_credit==1){
                      if(state.curr.grp.startsWith("IS0017Spring2020")) return "./img/half_credit.png"; //icon specific for IS0017 Fall 2019 (do not use blue as it is the color for group progress)
...
function generateHelp(origin){
    ...
    if(origin === "one-res-me-h"){
		...
		if(state.args.uiIncenCheck) {
      if (state.curr.grp.startsWith("IS0017Spring2020")){
