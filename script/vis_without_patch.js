...
const LANGUAGES = {
  en: {
    ...
    hard:"hard",
    high:"high",
    descriptionDifficultyGauge: "<h3 style='margin: 0px; padding: 0px 10px 0px 0px;'>Activity's difficulty</h3><p>This gauge estimates the difficulty of a learning activity based on the level of knowledge required to attempt it and your current level of knowledge.</p>",
    helpTextExtraPoints: "<h3>Points per Topic</h3><img src='./img/credit.png' alt='Full credit' width='15' height='15' style='display:inline;'><p style='display:inline;'>means that you got 2 points for completing at least 2 Coding problems.</p><br><img src='./img/half_credit.png' alt='Half credit' width='15' height='15' style='display:inline;'><p style='display:inline;'>means that you got 1 point for completing at least 1 Coding problem.</p><br><img src='./img/no_credit.png' alt='No credit' width='15' height='15' style='display:inline;'><p style='display:inline;'>means that you have not completed any Coding problem in this topic.</p>",
  ...
  es: {
    ...
    helpTextExtraPoints: "<h3>Puntos por unidad</h3><img src='./img/credit.png' alt='Crédito completo' width='15' height='15' style='display:inline;'><p style='display:inline;'>significa que obtuviste 2 puntos por completar al menos 2 problemas de Codificación.</p><br><img src='./img/half_credit.png' alt='Medio crédito' width='15' height='15' style='display:inline;'><p style='display:inline;'>significa que obtuviste 1 punto por completar al menos 1 problema de Codificación.</p><br><img src='./img/no_credit.png' alt='Sin crédito' width='15' height='15' style='display:inline;'><p style='display:inline;'>significa que no has completado ningún problema de Codificación en esta unidad.</p>",
...
var max_total_recs = 9;//Number of recommended activities that will be shown at each time (added by @Jordan)
...
var max_recommendations_per_topic = 4;
var recTooltip;

//star drawing related variables
var scaleRecommendationStar =
  d3.scale.linear().
    domain([0, 1]).
    range([4, 10]);
var innerRatio = 0.5;      // inner tip radius / outer radius
var numPoints = 5;
//@AALTOSQL21
...
function actDone_cb(rsp) {
  ...
  if (data.configprops.agg_kc_student_modeling == "cumulate") {
    for (var i = 0; i < data.kcs.length; i++) {
      ...
      kcs_estimates[kc_name] = curr_kc.finalValue ? curr_kc.finalValue : (curr_kc.uk ? curr_kc.uk : data.learners[usr_index].state.kcs[kc_id].k);
      console.log("KC: " + kc_name + " estimate: " + kcs_estimates[kc_name]);
    ...
    if (data.configprops.agg_proactiverec_enabled) {
      if (data.configprops.agg_proactiverec_method == "remedial") {
        ...
        if (recommended_activities.length > max_total_recs) {
          ...
          var top_rec_list_last_index = max_total_recs;
      ...
      if (CONST.vis.seqStars) {
        g
          ...
          .attr("points", function (d) {
            ...
            var cy = 6;
            var pts = starPoints(cx, cy, R, r, numPoints);
            return ((d.actIdx === -1 || d.seq === 0) ? "0,0" : pts);
          })
          .attr("style", function (d) { return "fill: " + CONST.vis.colors.sequencing + ";"; })
          //.attr("style", function (d) { return "border: 1px solid #FFFFFF;"; })
          .attr("stroke", "white")
...
function starPoints(cx, cy, R, r, n) {
  ...
  var step = Math.PI / n;      // half the angle between outer tips
  for (var i = 0; i < 2 * n; i++) {
    var rad = (i % 2 === 0) ? R : r;
...
function generateLearningRecommendations() {
  ...
  //name of the function to call to generate the recommendations
  ...
  if (typeof window[generateRecFunction] === 'function') {
    if (state.args.learningGoal != "") {
      ...
      console.log("Recommended activities after removing future topics:")
      console.log(recommended_activities)

      //Limit the number of recommendations to show in the UI within one topic
      var recommended_activities_temp = []
      var recommendations_per_topic = {}
      for (var i = 0; i < recommended_activities.length; i++) {
        console.log(recommended_activities[i])
        var act_topic = recommended_activities[i].topic_name;
      ...
      console.log(recommendations_per_topic)

      recommended_activities = recommended_activities_temp;
      console.log(max_total_recs)
      if (recommended_activities.length > max_total_recs) {
        /*var top_rec_list_first_index = recommended_activities.length/2 - max_total_recs/2;
        if (top_rec_list_first_index<0){
          top_rec_list_first_index=0;
      ...
      //Here we get the maximum rank of the items recommended per topic
      for (var i = 0; i < top_recommended_activities.length; i++) {
        var rec_act_topic = top_recommended_activities[i]["topic_name"];
        var rec_act_name = top_recommended_activities[i]["name"];
        var rec_act_id = top_recommended_activities[i]["id"];
        if (!(rec_act_topic in map_topic_max_rank_rec_act)) {
          map_topic_max_rank_rec_act[rec_act_topic] = i;
...
function createConceptBarRow(d, label_top = false, add_checkbox = false, extra_info = false, editable = false) {
  ...
  if (add_checkbox) {
    ...
    checkbox.onclick = function () {
      ...
      //log to see if they checked or unchecked the box
      log(
        "action" + CONST.log.sep02 + "kc-select" + CONST.log.sep01 +
        "kc-name" + CONST.log.sep02 + d.n + CONST.log.sep01 +
        "checked" + CONST.log.sep02 + checkbox.checked + CONST.log.sep01 +
...
// --- PATCH: DisabledForRec styling and tooltip ---
  ...
  // If label_top is true, place labelContainer above barContainer
  if (label_top) {
    // Create a wrapper for labelContainer to be on top
    const labelTopDiv = document.createElement('div');
    labelTopDiv.appendChild(labelContainer);
    row.appendChild(labelTopDiv);
    // Only barContainer in the firstLine
    ...
    // Add the first line to the row (label and bar side by side)
...
// Function to remove activities from future topics which it is useful for not recommending 
function removeActivitiesFromFutureTopics(recommended_activities) {
  recommended_activities = recommended_activities.filter(function (rec) {
    const topicObj = data.topics.find(t => t.id === rec.topic_id || t.name === rec.topic_name);
...
// Add this helper function anywhere in the file (e.g., near the bottom)
