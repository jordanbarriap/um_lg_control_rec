...
var offsetTopActSvg = -200; //modified by @Jordan, originally was -120
...
//     }
     ...
     svg = d3.select("div#chart")
          ...
          nodename.enter().append("text")
            ...
            .attr("transform", function(d,i) {
                          return "translate("+((barWidth+barSeparation)*(orderMap[d.id])+barChartLeftPos )+", 270) rotate(45)";    
            ...
            /*svg.append("line")
             ...
             svg.append("line")
               .attr("class","trivial")
               .attr({ x1: barChartLeftPos, y1: (yStartBipartite+2), //start of the line
          ...
          for (var i=0;i<learner_kcs.length;i++){
            learner_kcs[i].uk=getMe().state.kcs[data.kcs[i].id].k;
          }
          ...
          //// console.log(learner_kcs);
           ...
           var bars = svg.selectAll("g.bar");
            ...
            var userbar = bars.enter().append("g")
                ...
                .attr("transform", function(d) {
                  ...
                  if(barSizeUser==0||barSizeUser==-1) return "translate("+(barWidth*orderMap[d.id]+barChartLeftPos)+","+ (yStartBipartite-5)+")";
                  else return "translate("+((barWidth+barSeparation)*orderMap[d.id]+barChartLeftPos)+","+ (yStartBipartite-barSizeUser)+")"; })
                ...
                .style("fill", function(d) {
                  ...
                  else return colorbrewer[barColorUserProperty][barColorScale][Math.round((barColorScale-1)*d.uk)]; })
                .style("stroke", function(d) { return "white"; })
                .style("stroke-width", 1)
                .style("cursor", "pointer");
                /*.attr("transform", function(d) {
            ...
            d3.selectAll("g.bar")//added by jbarriapineda today
              .append("rect")
              .attr("height", function(d) {
                    return maxBarHeight;
                  })
              ...
              .attr("transform", function(d) {
                  var barSizeUser = bipartiteBarScale(d.uk);
                  //if(d[barSizeUser]==0||d[barSizeUser]==-1) return "translate("+(barWidth*orderMap[d.id]+barChartLeftPos)+","+ (250-5)+")";
                  if(barSizeUser==0||barSizeUser==-1) return "translate("+0+","+ (5-maxBarHeight)+")";
                  //else 
            ...
            for (var j=0;j<data.kcs.length;j++){
           ...
           //this part is supposed to show the black dot differently than each other
            ...
            var diagonal = d3.svg.diagonal()
                  ...
                  .target(function(d) { return {"x":mapTopicPos[d.topicId], "y":10}; })
  ...
  if(orderBy=="topic"){
    ...
    for(var i=0;i<topics.length;i++){
      if(!topics[i].order){
        ...
        topic_order[topics[i].id]=topics[i].order;
      }
      
    }
    var kcs_copy=kcs.concat([]);
    sorted_kcs = kcs.sort(function(a, b){
      if (topic_order[a.t]  > topic_order[b.t]){
        return 1;
      }else{
        if(topic_order[a.t] == topic_order[b.t]){
          if(groupSorting=="me"){
...
function topicNodeMouseOver(tid,origin) {
  ...
  d3.selectAll(".bar").filter(function(d){return d.t==tid}).classed("active", true);
  d3.selectAll(".groupbar").filter(function(d){return d.t==tid}).classed("active", true);
  d3.selectAll(".nodename").filter(function(d){return d.t==tid}).classed("active", true);
  d3.selectAll(".text-userbar").filter(function(d){return d.t==tid}).classed("active", true);
...
function highlightKcsOnActivityMouseOver(actId,resIdx){
  if(uiCMVisId=="bipartite"){
    ...
    percent = kc_state_act.difficulty;
...
function hideSocialComparisonBipartite(){
  ...
  d3.selectAll(".nodename")
    ...
    .attr("transform",function(){
      ...
      var current_y = transform.translate[1];
      //return "translate("+current_x+","+(current_y+165)+") rotate(45)";});
      return "translate("+current_x+","+(yStartBipartite+10)+") rotate(45)";});
}
